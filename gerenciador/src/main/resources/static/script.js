...
function renderizarLista(medicos) {
  ...
  if (!medicos.length) {
    lista.innerHTML = '<div class="estado-vazio">Nenhum médico cadastrado.</div>';
    return;
  }
    ...
    card.innerHTML = `
      <div class="medico-info">
        <div class="medico-nome-linha">
          ${esc(m.nome)}
          ${m.statusEnum === "DESATIVADO" ? '<span class="badge-desativado">DESATIVADO</span>' : ""}
        </div>
        <div><strong>CRM:</strong> ${esc(m.crm)}</div>
        <div><strong>Especialidade:</strong> ${esc(m.especialidade)}</div>
      </div>
      <div class="medico-acoes">
        <button class="btn-alterar" onclick="abrirModalEditar(${m.id})">Alterar</button>
        <button class="btn-excluir" onclick="abrirConfirmar(${m.id}, '${esc(m.nome)}')">Excluir</button>
      </div>
    `;
...
async function carregarMedicos(especialidade = "") {
  const lista = document.getElementById("lista-medicos");
  lista.innerHTML = '<p style="text-align:center;font-size:13px;color:#555;padding:12px 0;">Carregando...</p>';

  try {
    const url = especialidade
      ? `${API}?especialidade=${encodeURIComponent(especialidade)}`
      : API;
    ...
    lista.innerHTML = '<p style="text-align:center;font-size:13px;color:#a94442;padding:12px 0;">Erro ao carregar médicos.</p>';
  }
}

function filtrar() {
  const termo = document.getElementById("inp-filtro").value.trim();
  carregarMedicos(termo);
}
...
async function abrirModalEditar(id) {
  try {
    ...
    document.getElementById("modal-form-titulo").textContent = "Alterar médico";
    document.getElementById("inp-nome").value = m.nome ?? "";
    document.getElementById("inp-crm").value = m.crm ?? "";
    document.getElementById("inp-especialidade").value = m.especialidade ?? "";
    document.getElementById("inp-desativado").checked = m.statusEnum === "DESATIVADO";
...
async function salvarMedico() {
  ...
  try {
    ...
    mostrarAlerta("Médico atualizado com sucesso!", "sucesso");
    carregarMedicos();
  } catch {
...
async function confirmarExcluir() {
  ...
  try {
    const res = await fetch(`${API}/${idExcluindo}`, { method: "DELETE" });
    if (!res.ok) throw new Error();

    fecharModal("modal-confirmar");
    mostrarAlerta("Médico excluído com sucesso!", "sucesso");
    carregarMedicos();
  } catch {
    mostrarAlerta("Erro ao excluir médico.", "erro");
  } finally {
    btn.disabled = false;
